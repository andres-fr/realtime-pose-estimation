...
Sources:
https://arxiv.org/abs/2003.04030
https://github.com/caiyuanhao1998/RSN/blob/master/exps/RSN18.coco/network.py
"""
...
class conv_bn_relu(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding,
                 has_bn=True, has_relu=True, checkpoint=False):
...
class ResidualStepBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None,
        ...
        self.conv_bn_relu1 = conv_bn_relu(
            in_planes, 4*self.branch_ch, kernel_size=1,
            stride=stride, padding=0, has_bn=True, has_relu=True,
            checkpoint=checkpoint)

        self.conv_bn_relu2_1_1 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_2_1 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_2_2 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_3_1 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_3_2 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_3_3 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_4_1 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_4_2 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_4_3 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)
        self.conv_bn_relu2_4_4 = conv_bn_relu(
            self.branch_ch, self.branch_ch, kernel_size=3,
            stride=1, padding=1, has_bn=True, has_relu=True,
            checkpoint=checkpoint)

        self.conv_bn_relu3 = conv_bn_relu(
            4*self.branch_ch, planes * self.expansion,
            kernel_size=1, stride=1, padding=0, has_bn=True,
            has_relu=False, checkpoint=checkpoint)
        self.relu = nn.ReLU(inplace=True)
