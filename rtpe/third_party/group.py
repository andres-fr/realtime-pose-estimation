...
class Params(object):
    def __init__(self, num_joints, max_num_people, detection_threshold,
                 tag_threshold, use_detection_val, ignore_too_much):
        self.num_joints = num_joints  # cfg.DATASET.NUM_JOINTS
        self.max_num_people = max_num_people  # cfg.DATASET.MAX_NUM_PEOPLE

        self.detection_threshold = detection_threshold  # cfg.TEST.DETECTION_THRESHOLD
        self.tag_threshold = tag_threshold  # cfg.TEST.TAG_THRESHOLD
        self.use_detection_val = use_detection_val  # cfg.TEST.USE_DETECTION_VAL
        self.ignore_too_much = ignore_too_much  # cfg.TEST.IGNORE_TOO_MUCH
        self.joint_order = list(range(num_joints))
...
class HeatmapParser(object):
    def __init__(self, num_joints, max_num_people, detection_threshold,
                 tag_threshold, use_detection_val, ignore_too_much,
                 tag_per_joint=True, nms_ksize=5, nms_padding=2):
        self.params = Params(num_joints, max_num_people, detection_threshold,
                             tag_threshold, use_detection_val, ignore_too_much)
        self.tag_per_joint = tag_per_joint  # cfg.MODEL.TAG_PER_JOINT
        self.nms_pool = torch.nn.MaxPool2d(nms_ksize, 1, nms_padding)

    def nms(self, det):
        maxm = self.nms_pool(det)
    ...
    def parse(self, det, tag, adjust=True, refine=True):
        ...
        if adjust:  #  "move by a quarter" heuristic (see DARK paper)
