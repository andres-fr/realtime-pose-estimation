...
class AttentionStudent(torch.nn.Module):
    """
    This model was able to pick up augmented train and val fairly well. It
    tends to show false positives, but most humans are captured, and almost
    no false negatives. Good candidate for refinement!
    """
    def __init__(self, hhrnet_statedict_path=None, device="cuda",
                 inplanes=48,
                 # num_stages=3,
...
class AttentionStudentSteps(torch.nn.Module):
    """
    Here, the attention map is just concatenated to the mid stem,
    and the capacity of the keypoint detector enhanced.

    TODO:

    * Add a few inplanes to the mid-stem output
    * Attention map is concatenated to mid-stem output, like in OpenPose
    * Increase detector capacity. Add step-convs instead of CAMs?
    * What about the balance between mask and HM optimizer?

    * Loss fn: The attention maps are added TO THE END OF EACH STAGE, like
      in the progressive context refinement paper.
    """
    def __init__(self, hhrnet_statedict_path=None, device="cuda",
                 inplanes=48,
                 # num_stages=3,
        ...
        self.att_lo, self.att_mid, self.att_hi, self.att_top = self._attention_body()
        # self.det_lo, self.det_mid, self.det_hi, self.det_top = self._detection_body_v1()
        self.steps = self._detection_stage()
        # self.det = self._detection_body()
        # self.cams, self.hm_convs = self._make_body()
        # optionally initialize parameters
        if init_fn is not None:
    ...
    def _detection_stage(self):
        ...
        #     torch.nn.AvgPool2d(kernel_size=3, stride=2, padding=1,
        #                        count_include_pad=False),
        #     ContextAwareModule(self.inplanes + 1, hdc_dilations=[1, 2, 3, 4]),
        # )
        # #
        # mid_res = torch.nn.Sequential(
        #     torch.nn.AvgPool2d(kernel_size=3, stride=2, padding=1,
        ...
        # )
        # top = torch.nn.Sequential(
        #     torch.nn.Conv2d(self.inplanes + 1, hm_out_ch, kernel_size=3,
        #                     stride=1, dilation=1, padding=1,
        #                     bias=True))
        # #
    ...
    def forward(self, x, out_hw=None, alt=None, att_divisor=20):
        ...
        # # THIS DOMINATED THE KEYPOINTS, BUT MAY BE BC LACK OF CAPACITY
        # stem_out = stem_out + att.expand(stem_out.shape)
