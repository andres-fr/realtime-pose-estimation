...
#
from rtpe.third_party.utils import get_model_summary
...
from rtpe.students import RefinerStudent, MultistageStudent
...
VAL_GT_STDDEVS = [2.0]
TRAIN_GT_STDDEVS = [20.0, 5.0]
DISTILLATION_ALPHA = 0.5
OPT_INIT_PARAMS = {"momentum": 0.9, "weight_decay": 0.000}
...
# minival
TB_DIAGNOSE_EVERY_BATCHES = 10
MINIVAL_EVERY_BATCHES = 1500
...
student = RefinerStudent(MODEL_PATH, DEVICE, LAYERS_PER_STAGE,
                         NUM_HEATMAPS, AE_DIMENSIONS,
                         HALF_PRECISION,
                         PARAM_INIT_FN,
                         TRAINABLE_STEM,
                         BATCHNORM_MOMENTUM)
...
student_summary = get_model_summary(student, DUMMY_INPUT, verbose=True)
...
#         "Layer", ["name", "input_size", "output_size", "num_parameters",
#                   "multiply_adds"])
...
# # TODO FINISH THIS RECURSIVE MODEL SUMMARY

# # ALSO CHECK BETTER LEARNING RATE. AND MAYBE TRAIN WITH 1K IMAGES TO SEE IF AT LEAST THE THING LEARNS

# def dbg(module): print(module)

# dbg = lambda module: print(module.__class__.__name__)
...
for epoch in range(NUM_EPOCHS):
    for (img_id, imgs, masks, hms, teach_hms, teach_aes) in train_dl:
        ...
        #
        imgs = imgs.to(DEVICE)
        gt = torch.cat([hms[0], teach_aes[:, 0:1, :, :]], dim=1).to(DEVICE)
        masks = masks.unsqueeze(1).expand(*gt.shape).to(DEVICE)
        teach_preds = torch.cat([teach_hms, teach_aes[:, 0:1, :, :]],
                                dim=1).to(DEVICE)
        #
        preds = student(imgs, out_hw=TRAIN_HW)

        batch_loss = loss_fn(preds, teach_preds, gt,
        ...
        if global_step % TB_DIAGNOSE_EVERY_BATCHES == 0:
            ...
            tb_logger.add_image("GT heatmaps",
                                hms[0][0].max(dim=0)[0].unsqueeze(0),
                                global_step=global_step)
            ...
            tb_logger.add_images("predictions", tb_preds,
                                 global_step=global_step,
                                 dataformats="CHW")
